...
- **Ràng buộc đăng nhập**: `yeu_cau_dang_nhap()` kiểm tra session, trả `chua_dang_nhap`. Đăng nhập yêu cầu CAPTCHA từ lần sai thứ 3 trở đi, giới hạn 5 lần sai → khóa 10 phút, trả `qua_so_lan` kèm số lần/khoá đến; có bảng `reset_khoa` để skip khóa. Đăng ký tài khoản cũng giới hạn 5 lần thử/10 phút mỗi IP. Cả hai bộ đếm lưu bền trong bảng `dem_thu_that_bai` (lib/gioi_han_toc_do.php) chứ không dùng `$_SESSION` - một bộ đếm theo session bị vô hiệu hoàn toàn bởi client không giữ cookie giữa các lần thử.
