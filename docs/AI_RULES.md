...
- **Xác thực & phân quyền**: API yêu cầu session đăng nhập (`yeu_cau_dang_nhap()`); không có vai trò quản trị toàn cục — mỗi giáo viên chỉ thấy/sửa lớp mình sở hữu (`lop_duoc_gan`, `kiem_tra_quyen_lop`) và lý do/quà do chính mình tạo (lọc theo `nguoi_tao_id`). Giáo viên chưa sở hữu lớp nào KHÔNG được cho qua — phải bị từ chối, không "tạm cho qua" như thiết kế cũ. Đăng ký/đăng nhập giới hạn 5 lần sai → khóa 10 phút, có cookie ghi nhớ (`gv_u`, `gv_t`).
