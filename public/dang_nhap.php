...
<h5 class="mb-3">Giao vien dang nhap</h5>
<div class="mb-2"><label class="form-label">Tai khoan</label><input id="u" class="form-control" value="gv1"></div>
<div class="mb-2"><label class="form-label">Mat khau</label><input id="p" type="password" class="form-control" value="123456"></div>
<button id="btn" class="btn btn-primary w-100">Dang nhap</button><div id="msg" class="small text-danger mt-2"></div>
</div></div></div></div></div>
<script>
btn.onclick = async () => {
  ...
  const j = await r.json(); if (j.ok) location.href='/public/trang_chinh.php'; else msg.textContent='Sai tai khoan hoac mat khau';
};
</script></body></html>
