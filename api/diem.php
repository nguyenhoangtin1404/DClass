...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && $hanh_dong === 'lich_su') {
  ...
  $lop_duoc_gan = lop_duoc_gan($pdo, $gv_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && $hanh_dong === 'thong_ke') {
  ...
  $pr = $lop_gan;
  $where_lop = 'WHERE ' . implode(' AND ', $cond_lop);
  // Top số dư hiện tại
  ...
  // Tồn kho quà (chỉ quà của chính giáo viên này)
  $stTon = $pdo->prepare("SELECT id, ten, gia_diem, ton_kho
                          FROM qua_tang
                          WHERE dang_hoat_dong=1 AND nguoi_tao_id=?
                          ORDER BY CASE WHEN ton_kho < 0 THEN 1 ELSE 0 END ASC, ton_kho ASC, ten ASC");
  $stTon->execute([$gv_id]); $ton_kho = $stTon->fetchAll();
  // Quà được yêu thích (đổi nhiều nhất), giới hạn trong lớp + quà của giáo viên này
  ...
  $sqlQuaUa = "SELECT qt.id, qt.ten,
               ...
               JOIN hoc_sinh hs ON hs.id = sc.hoc_sinh_id
               WHERE hs.lop_hoc_id IN ($place_q) AND qt.nguoi_tao_id = ?
               GROUP BY qt.id
               ORDER BY so_lan DESC, tong_diem DESC
               LIMIT 10";
  $st = $pdo->prepare($sqlQuaUa); $st->execute([...$lop_gan, $gv_id]); $qua_ua_thich = $st->fetchAll();
  json_phan_hoi(true, [
    'top_so_du' => $top_so_du,
