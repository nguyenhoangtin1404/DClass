...
if ($method === 'GET') {
  $st = $pdo->prepare("SELECT id, ten, gia_diem, ton_kho, dang_hoat_dong, anh_url FROM qua_tang WHERE nguoi_tao_id=? ORDER BY dang_hoat_dong DESC, gia_diem ASC, ten ASC");
  $st->execute([$gv_id]);
...
if ($method === 'POST') {
  ...
  if ($hanh_dong === 'them') {
    ...
    $stTrung = $pdo->prepare('SELECT id FROM qua_tang WHERE ten = ? COLLATE NOCASE AND nguoi_tao_id = ? LIMIT 1');
    $stTrung->execute([$ten, $gv_id]);
  ...
  if ($hanh_dong === 'sua') {
    ...
    $co = $pdo->prepare('SELECT 1 FROM qua_tang WHERE id=? AND nguoi_tao_id=?');
    $co->execute([$id, $gv_id]);
    ...
    if ($ten !== null) {
      if ($ten === '') return json_phan_hoi(false, null, 'thieu_ten');
      $stTrung = $pdo->prepare('SELECT id FROM qua_tang WHERE ten = ? COLLATE NOCASE AND nguoi_tao_id = ? AND id <> ? LIMIT 1');
      $stTrung->execute([$ten, $gv_id, $id]);
      if ($stTrung->fetchColumn()) return json_phan_hoi(false, null, 'ten_da_ton_tai');
      $set[]='ten=?'; $pr[]=$ten;
    }
    if ($gia_diem !== null) {
    ...
    ghi_log($pdo, $gv_id, 'sua_qua', 'Sửa quà id '.$id.($ten!==null?(' -> '.$ten):''));
  ...
  if ($hanh_dong === 'bat_tat') {
    ...
    ghi_log($pdo, $gv_id, 'bat_tat_qua', 'Bật/tắt quà id '.$id.' => '.($trang_thai?1:0));
    return json_phan_hoi(true);
  }
  if ($hanh_dong === 'xoa') {
    $id = (int)($b['id'] ?? 0);
    if ($id <= 0) return json_phan_hoi(false, null, 'thieu_id');
    $st = $pdo->prepare('SELECT anh_url FROM qua_tang WHERE id=? AND nguoi_tao_id=?');
    $st->execute([$id, $gv_id]);
    $row = $st->fetch();
    if (!$row) return json_phan_hoi(false, null, 'khong_du_quyen');
    ...
    ghi_log($pdo, $gv_id, 'xoa_qua', 'Xóa quà id '.$id);
    return json_phan_hoi(true);
  }
}
